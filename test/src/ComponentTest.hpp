...
TEST(ComponentList, AddRemove)
{
    ComponentList cl;
    EXPECT_EQ(cl.types.size(), 0);
    EXPECT_EQ(cl.types.find(Comp0::GetType()), cl.types.end());

    cl.Add<Comp0>();
    EXPECT_EQ(cl.types.size(), 1);
    EXPECT_EQ(cl.types.find(Comp0::GetType()), cl.types.begin());
...
TEST(ComponentList, ComponentListHash)
{
    ...
    std::unordered_map<ComponentList, int, ComponentListHash> map;
